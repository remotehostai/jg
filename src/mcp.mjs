...
export async function startMcp(root) {
  ...
  const server = new McpServer({ name: 'jevgrep', version: '0.2.0' }, { instructions: 'Use search_code to locate code by behavior when exact names are unknown. Use ripgrep for exact symbols, regex, and exhaustive references. Read returned source before editing. Shortlisted or skipped coverage cannot establish absence; narrow paths or request broad search. Source snippets go to the configured hosted Jev service.' });
  server.registerTool('search_code', {
    ...
    inputSchema: { query: z.string().min(1).max(2000), paths: z.array(z.string()).min(1).max(20).default(['.']), limit: z.number().int().min(1).max(10).default(5), threshold: z.number().min(0).max(1).default(0.7), broad: z.boolean().default(false), dry_run: z.boolean().default(false), no_cache: z.boolean().default(false) },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ query, paths, limit, threshold, broad, dry_run, no_cache }, extra) => {
    try {
      const resolved = [];
      ...
      const result = await search(query, resolved, { limit, threshold, broad, dryRun: dry_run, useCache: !no_cache, signal: extra.signal });
